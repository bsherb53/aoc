...
func (d data) valid() error {
	if d.birthYear == "" {
		return errors.New("birthYear")
	}
	i, err := strconv.Atoi(d.birthYear)
	if err != nil {
		return errors.New("birthYear")
	}
	if i < 1920 || i > 2002 {
	...
	if d.issueYear == "" {
		return errors.New("issueYear")
	}
	i, err = strconv.Atoi(d.issueYear)
	if err != nil {
		return errors.New("issueYear")
	}
	if i < 2010 || i > 2020 {
	...
	if d.expYear == "" {
		return errors.New("expYear")
	}
	i, err = strconv.Atoi(d.expYear)
	if err != nil {
		return errors.New("expYear")
	}
	if i < 2020 || i > 2030 {
	...
	// get measurement
	ms := d.height[len(d.height)-2:]
	msnum := d.height[0 : len(d.height)-2]
	switch ms {
	case "cm":
		i, err = strconv.Atoi(msnum)
		if err != nil {
			return errors.New("height")
		}
		if i < 150 || i > 193 {
	...
	if d.hairColor == "" {
		return errors.New("hairColor")
	}

	if len(d.hairColor) != len("#aaabbb") {
		return errors.New("hairColor")
	}

	r, _ := regexp.Compile("#[a-fA-F0-9]{6}")
	if m := r.Match([]byte(d.hairColor)); !m {
		return errors.New("hairColor")
	}

	if d.eyeColor == "" {
		return errors.New("eyeColor")
	}
	if d.eyeColor != "amb" &&
		d.eyeColor != "blu" &&
		d.eyeColor != "brn" &&
...
var sampleData = `iyr:2020 hgt:76in hcl:#b6352a ecl:blu byr:2001 eyr:2030 pid:000000000 cid:23432`

var dataString = `byr:1985
