...
func Do(log bool) {
	...
	for i := range groups {
		...
		for k, p := range strings.Split(strings.TrimSpace(groups[i]), "\n") {
			//fmt.Println(p)
			if k == 0 {
				for j := range p {
					if !contains(string(p[j]), groupAnswers) {
						groupAnswers = append(groupAnswers, string(p[j]))
					}
				}
			} else {
				for j := range p {
					// if the answer isn't in previous
					if !contains(string(p[j]), groupAnswers) {
						groupAnswers = remove(string(p[j]), groupAnswers)
					}

					// if the previous isn't in current
					for _, ga := range groupAnswers {
						if !strings.Contains(p, ga) {
							groupAnswers = remove(ga, groupAnswers)
						}
					}
				}
			}
...
func remove(i string, a []string) []string {
	n := make([]string, 0)
	for j := range a {
		if a[j] != i {
			n = append(n, a[j])
